...
<body class="font-poppins text-[#0E0140] pb-[100px] overflow-x-hidden">
    ...
    <nav class="container max-w-[1130px] mx-auto flex items-center justify-between pt-10">
        ...
        <ul class="flex items-center gap-10">
            <li>
                <a href="{{ route('home.index') }}" class="transition-all duration-300 hover:font-semibold hover:text-[#FF6B2C] font-medium text-white">Home</a>
            </li>
            <li>
    ...
    <form method="POST"  enctype="multipart/form-data" action="{{ route('home.apply.store', $companyJob->slug) }}" class="relative max-w-[900px] mx-auto flex flex-col rounded-[20px] bg-white border border-[#E8E4F8] p-[30px] gap-10 shadow-[0_8px_30px_0_#0E01400D] mt-[130px]">
        @csrf
        ...
        <div id="Feature" class="flex items-center gap-6">
            ...
            <div class="flex items-center gap-[6px]">
                ...
                <p class="font-semibold text-lg leading-[27px]">{{number_format($companyJob->salary, 0, ',', '.' )}}/mo</p>
            </div>
            <div class="flex items-center gap-[6px]">
                <div class="flex shrink-0 w-[38px] h-[38px]">
        ...
        @if($errors->any())
            <div class="bg-red-500 text-white py-3 w-full px-5">
